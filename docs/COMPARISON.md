...
| **SAP Business Accelerator Hub** | api.sap.com | Official SAP sandbox for hundreds of S/4HANA Cloud OData v2/v4 APIs; free `APIKey` from a SAP Community login; primary "live backend" path for SAP-Automate v1.3+ |
| **`modelcontextprotocol/spec`** | github.com/modelcontextprotocol | Canonical MCP 2025-06-18 specification — capability negotiation, optional utilities (logging, completion, progress, cancellation), HTTP transport security (origin validation, RFC 8707 Resource Indicators) |
...
| **MCP optional utilities** | spec defines `logging/setLevel`, `completion/complete`, `notifications/{progress,cancelled,message}`; absent in most reference SAP MCP servers | **All five wired** in `mcp-core` types + `mcp-server` handlers. `logging/setLevel` stores an atomic per-server level; `completion/complete` runs a pluggable per-prompt argument completer (registered for `sap.skill.security_sod_audit`, `…abap_code_review`, `…bw_to_datasphere_migration`). `notifications/progress` + `…/cancelled` wire types in place; tool-side emission lands in v1.4. 14 unit + integration tests. |
| **HTTP transport — DNS rebinding** | spec MUST §4.6; absent in most references | **`--allowed-origin <url>`** repeatable CLI flag; `check_origin()` enforces an exact-match allowlist on both `POST /mcp` and `GET /mcp/events`; mismatched / missing Origin → HTTP 403. 5 unit tests covering wildcard / matching / case-sensitive paths. |
| **Live SAP backend (sandbox)** | reference MCP servers either mock entirely (CData, mdk) or assume an on-prem destination (mario-andreschak, fr0ster); none ship a free public path | **`sap_automate_rfc::odata::BusinessHubClient`** against the SAP Business Accelerator Hub sandbox (`sandbox.api.sap.com`). OData v4 `API_BUSINESS_PARTNER` piloted; `APIKey` header auth from `SAP_BUSINESS_HUB_KEY` env (free SAP Community login); `sap.bp.search` + `sap.bp.get` MCP tools. Friendly "feature disabled" fallback when the env var is unset — capability discovery stays consistent. 8 unit + 1 live-skipping integration + 5 in-process MCP-tool tests. Three-tier integration strategy documented in [`docs/INTEGRATION.md`](INTEGRATION.md). |
